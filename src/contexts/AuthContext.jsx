...
const authAPI = { 
  login: async (credentials) => {
    ...
    if (credentials.phone) {
      ...
      if (otp.length !== 4) {
        throw new Error('Code OTP invalide (4 chiffres requis)')
      }

      // Pour la démo, tout OTP à 4 chiffres est accepté
      return {
        user: {
          id: phone,
          name: credentials.name || 'Agent Mobile',
          phone,
          role: 'SUPER_ADMIN',
          roles: ['SUPER_ADMIN', 'KYC_ADMIN', 'FUNDS_ADMIN']
        },
        accessToken: `token_${phone}`,
