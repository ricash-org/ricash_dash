...
// Définition complète de tous les items du menu
const allMenuItems = [
  {
    title: 'Dashboard',
    ...
    roles: ['ADMIN', 'AGENT'] // Accessible à tous
  },
  {
    title: 'Utilisateurs',
    icon: Users,
  ...
  },
  {
    ...
    roles: ['ADMIN', 'AGENT'] // Admin uniquement
  },
  {
    title: 'Agents',
    icon: UserCheck,
...
export default function Sidebar({ isOpen, onToggle }) {
  ...
  const { user, getAccessibleMenuItems } = usePermissions()

  // Filtrer les items selon le rôle
  const menuItems = getAccessibleMenuItems(allMenuItems)

  const isActive = (href) => {
    return location.pathname === href || location.pathname.startsWith(href + '/')
  }

  // Afficher un message de chargement si pas d'utilisateur
  if (!user) {
    return (
      <div className={cn(
              ...
              >
                ...
                >
                  <div className="flex items-center w-full">
                    ...
                    <div className={cn(
                      "w-9 h-9 flex items-center justify-center rounded-xl mr-3 transition-all duration-300",
                      "hover:bg-[#2B8286]/10 hover:shadow-md",
                      isActive(item.href) 
                        ? "bg-gradient-to-br from-[#2B8286] to-[#B19068] text-white shadow-lg" 
                        : "text-gray-500 hover:text-[#2B8286]"
                    )}>
