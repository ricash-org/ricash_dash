...
export default function Users() {
  ...
  // 🔥 CORRECTION : Fonction calculateTrend
  ...
  // 🔥 CORRECTION : Stats basées sur la VRAIE structure de données
  const stats = useMemo(() => {
    if (!users.length) {
      return {
        total: 0,
    ...
    return {
      total: users.length,
      // 🔥 CORRECTION : Utilisation de actif (booléen) au lieu de statut
      actifs: users.filter(u => u.actif === true).length,
      suspendus: users.filter(u => u.actif === false).length,
      // 🔥 CORRECTION : Utilisation de kycStatut au lieu de kycStatus
      kycEnAttente: users.filter(u => u.kycStatut === 'EN_COURS').length,
      kycRejetes: users.filter(u => u.kycStatut === 'REJETE').length,
      kycVerifies: users.filter(u => u.kycStatut === 'VERIFIE').length,
      kycNonVerifies: users.filter(u => u.kycStatut === 'NON_VERIFIE').length,
      // 🔥 CORRECTION : typeCompte n'existe pas dans vos données, utilisation de role
      premium: users.filter(u => u.role === 'PREMIUM' || u.role === 'premium').length,
      // 🔥 CORRECTION : Solde dans portefeuille.solde
      totalSolde: users.reduce((sum, u) => sum + (parseFloat(u.portefeuille?.solde) || 0), 0),
      // 🔥 CORRECTION : Transactions non disponibles dans vos données
      totalTransactions: users.reduce((sum, u) => sum + (parseInt(u.nombreTransactions) || 0), 0)
    };
  }, [users]);
  ...
  const statsData = useMemo(() => [
    ...
    },
    {
      ...
      icon: Clock,
      description: 'EN_COURS - À valider',
    ...
    },
    {
      ...
      icon: AlertTriangle,
      description: 'REJETE - Documents rejetés',
      color: '#ef4444'
    },
    {
      title: 'KYC Validés',
      value: stats.kycVerifies.toString(),
      change: calculateTrend(stats.kycVerifies).value,
      changeType: calculateTrend(stats.kycVerifies).type,
      icon: CheckCircle,
      description: 'VERIFIE - Documents approuvés',
      color: '#10b981'
    },
    {
      title: 'Solde Total',
  ...
  // 🔥 CORRECTION : Filtrage basé sur la VRAIE structure
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(user => {
      ...
      // 🔥 CORRECTION : Utilisation de actif (booléen) pour le statut
      const matchesStatus = filters.status === 'all' || 
        (filters.status === 'actif' && user.actif === true) ||
        (filters.status === 'suspendu' && user.actif === false);
      
      // 🔥 CORRECTION : Utilisation de kycStatut au lieu de kycStatus
      const matchesKyc = filters.kycStatus === 'all' || 
        user.kycStatut === filters.kycStatus;
      
      // 🔥 CORRECTION : Pays dans adresse.ville
      const matchesPays = filters.pays === 'all' || user.adresse?.ville === filters.pays;
      
      // 🔥 CORRECTION : typeCompte n'existe pas, utilisation de role
      const matchesType = filters.typeCompte === 'all' || user.role === filters.typeCompte;
...
const handleViewUser = useCallback((userId) => {
  ...
  // 🔥 CORRECTION : Gestion des statuts basée sur la VRAIE structure
  const getUserStatus = useCallback((user) => {
    // 🔥 CORRECTION : Utilisation de actif (booléen) pour le statut
    const isActif = user.actif === true;
    // 🔥 CORRECTION : Utilisation de kycStatut au lieu de kycStatus
    const kycStatus = user.kycStatut;
    ...
    return {
      status: {
        type: isActif ? 'success' : 'error',
        label: isActif ? 'Actif' : 'Suspendu'
      },
      kyc: {
        type: kycType,
        label: kycLabel
            ...
            <RicashTable>
              <RicashTableHeader>
                <RicashTableRow>
                  ...
                  <RicashTableCell className="font-semibold">Rôle</RicashTableCell>
                ...
                {filteredUsers.map((user) => {
                  ...
                  return (
                    <RicashTableRow key={user.id} className="hover:bg-[#376470]/5 transition-colors">
                      <RicashTableCell>
                        <div className="flex items-center space-x-3">
                          <div className={`w-2 h-2 rounded-full ${
                            userStatus.status.type === 'success' ? 'bg-green-500' : 'bg-red-500'
                          }`}></div>
                      ...
                      <RicashTableCell>
                        <div>
                          <div className="text-sm text-[#29475B]">{user.adresse?.ville || 'N/A'}</div>
                          <div className="text-sm text-[#376470]">{user.adresse?.codePostal || 'N/A'}</div>
                        </div>
                      </RicashTableCell>
                      ...
                      <RicashTableCell>
                        <div className="text-sm font-semibold text-[#29475B]">
                          €{(parseFloat(user.portefeuille?.solde) || 0).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </div>
                        <div className="text-xs text-[#376470]">{user.portefeuille?.devise || 'XOF'}</div>
                      </RicashTableCell>
                      ...
                      <RicashTableCell>
                        <div className="text-sm text-[#376470]">
                          {user.dateCreation ? new Date(user.dateCreation).toLocaleDateString('fr-FR') : 'Jamais connecté'}
                        </div>
                      </RicashTableCell>
                      ...
                      <RicashTableCell className="text-right">
                        ...
                        >
                          ...
                          {/* 🔥 CORRECTION : Condition avec kycStatut */}
                          {user.kycStatut === 'EN_COURS' && (
